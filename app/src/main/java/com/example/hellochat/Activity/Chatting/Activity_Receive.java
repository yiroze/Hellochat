...
public class Activity_Receive extends AppCompatActivity {
    TextView name;
    ImageView profile, connect, disconnect;
    String RoomID;
    int send_user_idx;
    private static final int CONNECTION_REQUEST = 1;
    private static final String TAG = "Activity_Receive";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        send_user_idx = intent.getIntExtra("send_user_idx", 0);
        ...
        connect.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Intent serviceIntent = new Intent(Activity_Receive.this, ClientService.class);
                try {
                    jsonObject.put("accept_user_idx", send_user_idx);
    ...
    private void connectToRoom(String roomId, boolean commandLineRun, boolean loopback,
        ...
        if (validateUrl(roomUrl)) {
            ...
            intent.setData(uri);
            intent.putExtra("user_idx", send_user_idx);
    ...
    public void getUserData(int send_user_idx) {
        LoginApi service = RetrofitClientInstance.getRetrofitInstance().create(LoginApi.class);
        Call<LoginData> call = service.getState(send_user_idx+"");
        call.enqueue(new Callback<LoginData>() {
            @Override
            public void onResponse(Call<LoginData> call, Response<LoginData> response) {
                loginData = response.body();
                String nameData = loginData.name;
                name.setText(nameData);
                String imageUrl = "http://3.37.204.197/hellochat/" + loginData.profile;
                Log.d(TAG, "onResponse: "+imageUrl);
                //영상 썸네일 세팅
                if(!loginData.profile.equals("")){
                    Glide.with(profile)
                            .load(imageUrl)
                            .into(profile);
                }
